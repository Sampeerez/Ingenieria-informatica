...
maze_t::is_ok_(const int i, const int j) const { 
  ...
  if (i >= 1 && j >= 1 && i <= matrix_.get_m() && j<= matrix_.get_n() && matrix_(i, j) != WALL_ID && !visited_(i, j)) return true;
  return false;
}

int maze_t::get_contador() const {
  return contador;
}
   ...
   for (int k = 0; k <= 3; ++k) {
    if (is_ok_(i + i_d[k], j + j_d[k])) {
      if (solve_(i + i_d[k], j + j_d[k])) {
        ...
        //solution_.push_front(new coord (pair_t<int>(i + i_d[k], j + j_d[k])));
        return true;
      }
    } else {
      ++contador;
...
//ostream& maze_t::ImprimirCamino(ostream& c) const {
//  return solution_.write(c);
//}
